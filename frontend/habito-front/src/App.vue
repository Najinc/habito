...
<template>
  <main class="min-h-screen bg-gradient-to-br from-indigo-100 via-slate-100 to-cyan-100 px-4 py-10">
    <section class="mx-auto w-full max-w-5xl space-y-6 rounded-3xl bg-white/80 p-6 shadow-soft backdrop-blur md:p-10">
      <header class="space-y-2">
        <p class="inline-flex rounded-full bg-indigo-50 px-3 py-1 text-sm font-medium text-indigo-700">
          Habito • Test Search
        </p>
        ...
        <p class="text-slate-600">
          Interface de test connectée au backend Python (`/api/search`) avec rendu des résultats Leboncoin.
        </p>
      </header>
      ...
      <section class="space-y-4">
        ...
        <div v-if="!hasResults && !isLoading" class="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center text-slate-500">
          ...
          >
            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div class="space-y-2">
                ...
                <p class="text-sm text-slate-600">{{ shortText(item.payload.body) }}</p>
              </div>
              <div class="flex shrink-0 items-center gap-2 text-sm">
                <span class="rounded-lg bg-emerald-50 px-2 py-1 font-medium text-emerald-700">
            ...
            <div class="mt-4 flex flex-wrap gap-2 text-sm text-slate-600">
              <span class="rounded-lg bg-slate-100 px-2 py-1">{{ formatPrice(item.payload.price) }}</span>
              <span class="rounded-lg bg-slate-100 px-2 py-1">{{ item.payload.city || 'Ville non renseignée' }}</span>
              <span class="rounded-lg bg-slate-100 px-2 py-1">{{ item.payload.square ?? 'N/A' }} m²</span>
              <span class="rounded-lg bg-slate-100 px-2 py-1">{{ item.payload.rooms ?? 'N/A' }} pièce(s)</span>
            </div>
