  ...
  <section class="space-y-4 rounded-3xl bg-white/90 p-5 shadow-soft md:p-7">
    <div class="flex items-center justify-between">
      ...
      <span class="rounded-full bg-slate-100 px-3 py-1 text-sm font-medium text-slate-600">
        {{ results.length }}<span v-if="allResultsCount > results.length"> / {{ allResultsCount }}</span>
        annonce(s)
      </span>
    </div>
            ...
            <div class="flex flex-1 flex-col gap-3">
              <div class="flex items-start justify-between gap-2">
                ...
                <div class="flex-shrink-0 text-sm">
                  <span class="whitespace-nowrap rounded-lg border px-2 py-1 font-medium" :style="scoreStyle(item.score)">
              ...
              <div class="flex flex-wrap gap-2 text-sm text-slate-600">
                ...
                <span class="rounded-lg bg-slate-100 px-2 py-1">{{ item.payload.city || "Ville non renseignee" }}</span>
                <span v-if="item.payload.square" class="rounded-lg bg-slate-100 px-2 py-1">{{ item.payload.square }} m2</span>
                <span v-if="item.payload.rooms" class="rounded-lg bg-slate-100 px-2 py-1">{{ item.payload.rooms }} piece(s)</span>
              </div>
            </div>
